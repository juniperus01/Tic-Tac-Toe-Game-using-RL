...
class InstructionsMenu:
    ...
    def display(self):
        ...
        instructions = [
            ...
            "Press '2' to play at Level 2 (Alpha-Beta Pruning AI)",
        ]
        y = self.height // 4
...
class AI:
    ...
    def alpha_beta_pruning(self, board, maximizing, alpha, beta):
        # print("using alpha-beta pruning AI")
        # terminal case
        case = board.final_state()
        ...
        if maximizing:
            ...
            for (row, col) in empty_sqrs:
                temp_board = copy.deepcopy(board)
                temp_board.mark_sqr(row, col, 1)
                eval = self.alpha_beta_pruning(temp_board, False, alpha, beta)[0]
                if eval > max_eval:
                    max_eval = eval
                    best_move = (row, col)
            ...
            for (row, col) in empty_sqrs:
                temp_board = copy.deepcopy(board)
                temp_board.mark_sqr(row, col, self.player)
                eval = self.alpha_beta_pruning(temp_board, True, alpha, beta)[0]
                if eval < min_eval:
                    min_eval = eval
                    best_move = (row, col)
                beta = min(beta, eval)
    ...
    def eval(self, main_board):
        ...
        if self.level == 0:
            ...
            print(f"Minimax computation time: {time.time() - start_time:.4f} seconds")

        else:
            # Alpha-Beta Pruning choice
            eval, move = self.alpha_beta_pruning(main_board, False, -float('inf'), float('inf'))
            print(f"Alpha-Beta Pruning computation time: {time.time() - start_time:.4f} seconds")
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        ...
                        if event.key == pygame.K_0:
                            ai.level = 0
                            print("Using Random AI (Level 0)")

                        
                        # 1 - unbeatable ai
                        if event.key == pygame.K_1:
                            ai.level = 1
                            print("Using Minimax AI (Level 1)")

                        if event.key == pygame.K_2:
                            ai.level = 2
                            print("Using Alpha-Beta Pruning AI (Level 2)")
